...
impl AtanValidator {
    ...
    fn calculate_active_lanes_root(&self, chain_block: &ChainBlock, mergeset_context_hash: Hash) -> AtanResult<Hash> {
        ...
        if let Some(expected_lane_id) = self.lane_id {
            if expected_lane_id != first_lane.lane_id {
        ...
        return Ok(first_lane_active_lanes_root);
    }
}
...
fn calculate_active_lanes_root_for_lane(lane_activity_digests_with_proof: &LaneActivityDigestsWithProof, mergeset_context_hash: Hash) -> AtanResult<Hash> {
    ...
    let lane_payload = smt_leaf_hash(&SmtLeafInput { lane_key: &lane_key, lane_tip: &lane_tip, blue_score: proof.last_touched_blue_score });
