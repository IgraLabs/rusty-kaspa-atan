        ...
        match self.store.get_node(&branch_key)? {
            Some(Node::Internal(_)) => {
                if depth == DEPTH - 1 {
                    ...
                    match self.store.get_leaf(&sibling_leaf_key)? {
                        ...
                        Some(leaf_hash) => {
                            Ok(Sibling(Some(hash_node::<H::CollapsedHasher>(sibling_leaf_key, leaf_hash))))
                        }
                    }
                } else {
                    ...
                    match self.store.get_node(&sibling_key)? {
                        ...
                        Some(Node::Internal(hash)) => {
                            Ok(Sibling(Some(hash)))
                        }
                        Some(Node::Collapsed(cl)) => {
                            Ok(Sibling(Some(hash_node::<H::CollapsedHasher>(cl.lane_key, cl.leaf_hash))))
                        }
                    }
                }
            }
            Some(Node::Collapsed(cl)) => {
                if cl.lane_key == branch_key.node_key {
                    Ok(Terminal(ProofTerminal::Collapsed { depth: depth as u8 }))
                } else {
                    Ok(Terminal(ProofTerminal::CollapsedOther { depth: depth as u8, leaf: cl }))
                }
            }
            None => {
                Ok(EmptySubtree)
